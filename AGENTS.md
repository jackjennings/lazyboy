...
`~/.lazyboy/tick.log` uses NDJSON format — one JSON object per line, each with a
`ts` field (ISO 8601 UTC) and an `event` field. The four events are:
...
`tick.log`. The tick process owns its own writes. Do not add
`>> ... tick.log 2>&1` back to the cron line.
