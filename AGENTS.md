...
`gh pr view --json url,title,baseRefName,headRefName` for each, derives
...
to `needs-attention` with reason `no-prs-in-output`. The action is idempotent:
once `prs` is non-empty it never fires again.
