...
This file provides guidance to coding agents (Claude Code, Pi, etc.) working in this repository.
...
Runtime env vars required for `tick`: `GITHUB_TOKEN`, `GITHUB_LOGIN`, `ANTHROPIC_API_KEY`. Config is read from `~/.config/lazyboy/config.toml`.
...
- Phase names are the single source of truth in `PHASE_SEQUENCE` (`src/phases/types.ts`). The Phase union type in `src/state/types.ts` must stay in sync with it manually.
- `waiting-diff` is not derived from `running-implementation` by the normal `running-X → waiting-X` pattern — it's a hardcoded special case in `runningPhaseToWaiting`.
- The state dir is a separate git repo (`~/code/jackjennings/projects` by default). `commitState` runs `git add -A && git commit` inside it after each tick.
