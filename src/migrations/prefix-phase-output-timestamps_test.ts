...
Deno.test("migration: renames suffixed revision file to prefixed format", async () => {
  ...
  try {
    ...
    assertEquals(entries.includes("20260629T225507-spec.md"), true);
    assertEquals(entries.includes("spec-20260629T225507.md"), false);
  } finally {
...
Deno.test("migration: renames hyphenated-date feedback file to prefixed format", async () => {
  ...
  try {
    ...
    const entries: string[] = [];
    for await (const e of Deno.readDir(ticketDir)) entries.push(e.name);
    assertEquals(entries.includes("20260629T224653-spec-feedback.md"), true);
    assertEquals(
      entries.includes("spec-feedback-2026-06-29T224653.md"),
      false,
    );
  } finally {
    await Deno.remove(stateDir, { recursive: true });
...
Deno.test("migration: renames canonical phase file using mtime when not committed", async () => {
  ...
  try {
    ...
    assertEquals(renamed !== undefined, true);
    assertEquals(entries.includes("spec.md"), false);
  } finally {
    await Deno.remove(stateDir, { recursive: true });
  }
});
