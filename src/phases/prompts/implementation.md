...
capture group or match. For string-presence tests (`content.includes("x") ===
true/false`): before writing any file whose content the plan specifies verbatim,
scan every test assertion about that file's content and verify the proposed text
satisfies it — especially `includes(...) === false` checks, which are easy to
violate when the plan's proposed text and its own tests were written in separate
sections. If any test input fails the planned regex or boundary condition, adjust
the implementation (not the test) to make it pass — the tests define the correct
behavior. Resolving plan inconsistencies before coding avoids a
test-fail-fix-retest cycle that costs three or more turns per discrepancy.
