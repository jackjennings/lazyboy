...
import { pushState } from "./push.ts";

const ok = { code: 0, stdout: "", stderr: "" };
const fail = { code: 1, stdout: "", stderr: "error" };

Deno.test("pushState: skips push when no origin remote", async () => {
  const runGit = spy((_args: string[], _cwd: string) => Promise.resolve(fail));
  const log = spy((_entry: object) => Promise.resolve());
  await pushState({ stateDir: "/state", runGit, log });
  assertSpyCalls(runGit, 1);
  assertSpyCalls(log, 0);
});

Deno.test(
  "pushState: pushes when origin exists and logs nothing on success",
  async () => {
...
Deno.test("pushState: logs error event when push fails", async () => {
...
Deno.test(
  "pushState: uses stateDir as cwd for both git calls",
  async () => {
    const cwds: string[] = [];
    ...
    const log = spy((_entry: object) => Promise.resolve());
    await pushState({ stateDir: "/my/stateDir", runGit, log });
    assertEquals(cwds, ["/my/stateDir", "/my/stateDir"]);
  },
);

Deno.test(
  "pushState: passes correct args to check and push commands",
  async () => {
