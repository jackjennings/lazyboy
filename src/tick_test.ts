...
Deno.test(
  "advancePhase: turns >= maxTurns transitions to needs-attention",
  async () => {
    const ticket = makeTicket({ phase: "spec", status: "running" });
    const writtenTickets: TicketState[] = [];
    const logs: object[] = [];
    ...
    assertEquals(
      (invalidLog as Record<string, unknown>).reason,
      "max-turns-exceeded",
    );
    assertEquals((invalidLog as Record<string, unknown>).turns, 50);
    assertEquals((invalidLog as Record<string, unknown>).maxTurns, 50);
  },
);
...
Deno.test(
  "advancePhase: turns < maxTurns falls through to output checks",
  async () => {
    const ticket = makeTicket({ phase: "spec", status: "running" });
    const writtenTickets: TicketState[] = [];
    await advancePhase(ticket, "/state", {
...
Deno.test(
  "advancePhase: readPhaseUsage returns null does not trigger max-turns check",
  async () => {
    const ticket = makeTicket({ phase: "spec", status: "running" });
    const writtenTickets: TicketState[] = [];
    await advancePhase(ticket, "/state", {
      ...
      readPhaseExitCode: () => Promise.resolve(0),
      readPhaseOutput: () => Promise.resolve("## What to Build\n\ncontent"),
      readPhaseUsage: () => Promise.resolve(null),
      maxTurns: 50,
    });
    const final = writtenTickets[writtenTickets.length - 1];
    assertEquals(final.status, "waiting");
...
Deno.test(
  "advancePhase: usage with undefined turns does not trigger max-turns check",
  async () => {
    const ticket = makeTicket({ phase: "spec", status: "running" });
    const writtenTickets: TicketState[] = [];
    await advancePhase(ticket, "/state", {
...
Deno.test(
  "advancePhase: readPhaseUsage absent does not trigger max-turns check",
  async () => {
    const ticket = makeTicket({ phase: "spec", status: "running" });
    const writtenTickets: TicketState[] = [];
    await advancePhase(ticket, "/state", {
...
Deno.test(
  "advancePhase: maxTurns absent does not trigger max-turns check even when usage present",
  async () => {
    const ticket = makeTicket({ phase: "spec", status: "running" });
    const writtenTickets: TicketState[] = [];
    await advancePhase(ticket, "/state", {
